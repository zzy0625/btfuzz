...
void custom_log(const char* message) {
    const char* log_file_path = "bt-fuzzer.log";
    FILE* log_file = fopen(log_file_path, "a"); // 以追加模式打开日志文件
    if (log_file) {
        fprintf(log_file, "%s\n", message);
        fclose(log_file); // 记得关闭文件
    } else {
        fprintf(stderr, "Failed to open log file %s: %s\n", log_file_path, strerror(errno));
        // 如果文件打开失败，输出错误信息
    }
}
void log_packet(uint64_t packet_number, const char* data, size_t size) {
    // 将数据包内容写入日志文件
    FILE* log_file = fopen("bt-fuzzer.log", "a");
    if (log_file) {
        fprintf(log_file, "\nPacket #%llu - data (ASCII): ", packet_number);
        for (size_t i = 0; i < size; i++) {
            fprintf(log_file, "%02X ", (unsigned char)data[i]);
        }
        fprintf(log_file, "\n");
        fprintf(log_file, "Packet data (ASCII): ");
        for (size_t i = 0; i < size; i++) {
...
int open_l2cap_socket(int *psock, bdaddr_t *bdaddr) {
    ...
    *psock = socket(PF_BLUETOOTH, SOCK_RAW, BTPROTO_L2CAP);
    ...
    printf("%s",target_dev);
    printf("Fuzzing l2cap remote device %s via local device %s\n", target_dev, local_dev);
...
void fuzz(uint64_t iterations, bdaddr_t *bdaddr, int proto, int rfcomm_channel, int verbose_flag) {
    ...
    if (!fuzz_data) {
        perror("Failed to allocate fuzz data");
        return;
    }
	...
	int sent_bytes = send(sock, fuzz_data, size, 0);
        if  (sent_bytes<= 0) {
	    fprintf(stderr, "Error sending packet %llu: %s (%d)\n", i, strerror(errno), errno);
	    //log_packet(fuzz_data, size);
            error_occurred = 1; // 设置错误标志
            if (errno == ECONNRESET) {
    ...
    if (error_occurred) {
        printf("An error occurred during fuzzing.\n");
    }
...
int main(int argc, char *argv[]) {
    ...
    if (error_occurred) {
        custom_log("An error occurred during fuzzing, check logs for details.");
    }
    return 0;
}
